		...
		while(!this.sleep && this.fetchFillCount > 0
				...
				&& this.ifId_latch.isFull() == false)
	        	{
					...
					if(ins.getOperationType()==OperationType.sync)
					{
						...
						if(this.core.TreeBarrier == true){
							...
							if(bar.timeToCross())
							{
								...
								if(this.core.barrierUnit == 0){
									if(LocalClockperSm.getCurrentTime() < bar.time + 35)
									{
										bar_lat = (int)(this.core.barrier_latency + LocalClockperSm.getCurrentTime() - bar.time);
									}
									else
										bar_lat = this.core.barrier_latency;
								}
								else{
									if(LocalClockperSm.getCurrentTime() < bar.time + 4)
									{
										bar_lat = (int)(this.core.barrier_latency + LocalClockperSm.getCurrentTime() - bar.time);
									}
									else
						...
						this.ifId_latch.add(ins, LocalClockperSm.getCurrentTime() + 1);
						this.fetchFillCount--;
						...
						{
							System.out.println("fetched : " + LocalClockperSm.getCurrentTime()/core.getStepSize() + "\n"  + ins + "\n");
						}
					}
			}
