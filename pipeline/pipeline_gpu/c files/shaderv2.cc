...
void  issue_warp( register_set& pipe_reg_set, const warp_inst_t* next_inst, const active_mask_t &active_mask, unsigned warp_id )
{
...
void sfu::issue( register_set& source_reg )
{
	...
	m_core->incsfu_stat(m_core->get_config()->warp_size,(*ready_reg)->latency);
	 Pipeline Functionissue(source_reg);
}

void ldst_unit::active_lanes_in_pipeline(){
	unsigned active_count= Pipeline Functionget_active_lanes_in_pipeline();
...
void sp_unit :: issue(register_set& source_reg)
{
	...
	m_core->incsp_stat(m_core->get_config()->warp_size,(*ready_reg)->latency);
	 Pipeline Function  issue(source_reg);
}


 Pipeline Functionpipelined_simd_unit( register_set* result_port, const shader_core_config *config, unsigned max_latency,shader_core_ctx *core )
...
void  Pipeline Functionissue( register_set& source_reg )
{
...
void ldst_unit:: issue( register_set &reg_set )
{
   ...
   if (inst->is_load() and inst->space.get_type() != shared_space) {
	...
	m_core->incmem_stat(m_core->get_config()->warp_size,1);
	 Pipeline Functionissue(reg_set);
}
...
simt_core_cluster::simt_core_cluster( class gpgpu_sim *gpu, unsigned cluster_id, const struct shader_core_config *config,const struct memory_config *mem_config,
                                      shader_core_stats *stats, class memory_stats_t *mstats )
{
    m_config = config;
...
void  checkExecutionStatusAndUpdate(warp_inst_t &inst, unsigned t, unsigned tid)
{
    if(inst.isatomic())
